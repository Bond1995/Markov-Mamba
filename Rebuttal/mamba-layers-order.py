...
for order in range(1, 5):
    losses = []
    for run in api.runs("mamba-markov/markov-mamba-order-deep", {"$and":[{"config.order": order}, {"config.n_layer": 1}]}):
...
for n_layer, color in zip(range(2, 5), colors):
    ...
    for order in range(1, 5):
        losses = []
        for run in api.runs("mamba-markov/markov-mamba-order-deep", {"$and":[{"config.order": order}, {"config.n_layer": n_layer}]}):
            h = run.history(samples=25000)
            loss = h["val/loss_gap"].values[:]
            loss_min = np.nanmin(loss)
            losses.append(loss_min)
...
fig.savefig("mamba-layers-order.pdf", bbox_inches='tight')