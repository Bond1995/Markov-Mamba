...
for vocab_size, color in zip([2, 4, 6, 8], ["tab:blue", "tab:orange", "tab:green", "tab:purple"]):
    ...
    for h in df:
        ...
        opt_loss = h["val/opt_loss"].values[:]
        opt = np.nanmean(opt_loss)
        losses.append(loss / opt)
...
ax.set(xlabel="Iteration (x 200)", ylabel="Loss gap relative to optimal")
...
fig.savefig("mamba-states-relative-loss.pdf", bbox_inches='tight')