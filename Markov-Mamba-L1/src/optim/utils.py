...
@torch.no_grad()
def eval_probs(model, P, type, order, sequence_length, windows, generator, extra_args, input_seq=None, output_seq=None, ctx=nullcontext()):
    assert model.training == False
    assert P is not None
    ...
    est_vec = []
    for w in windows:
        est_vec.append(empirical_est(x, y, type, order, w))
